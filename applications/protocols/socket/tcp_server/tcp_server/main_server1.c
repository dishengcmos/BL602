...
//server
#define ROUTER_SSID "aithinker123"
#define ROUTER_PWD "123456789"
...
{
	char data[50]= "this is server send data!";
	static char data1=0x00;
    ...
    int ret = 0;
    char recievedata[1024] = { 0 };
    while (1) 
	{
	  	tcp_server_send(tcp_client_msg->socket_id, data);
	  	//tcp_server_send(tcp_client_msg->socket_id, data1);
		data1++;
        ret = tcp_server_receive(tcp_client_msg->socket_id, recievedata);	
        if (ret>0) 
		{
            printf("%s:%s\r\n", inet_ntoa(socket_addr->sin_addr.s_addr), recievedata);
			 
            //tcp_server_send(tcp_client_msg->socket_id, recievedata);
	  		//tcp_server_send(tcp_client_msg->socket_id, "have recieved data,now close");
...
void main()
{
    ...
    puts("HELLO,I protect to connect clinent! \r\n [OS] proc_main_entry task...\r\n");
